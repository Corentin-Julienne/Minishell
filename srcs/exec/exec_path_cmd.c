...
/*   Updated: 2022/06/10 18:12:45 by cjulienn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* 
First, try to check wether or not the user inputed a valid command with path.
Then, if it is not the case, split the arg str,
then recuperating path in PATH cmd, if no path provided,
because the env variable path was suppressed,
...
void	path_cmd_exec(t_shell *shell, char **cmd_args)
{
	...
	if (is_path_functionnal(cmd_args[0], shell, cmd_args) == 0)
		execve(cmd_args[0], cmd_args, shell->env);
